...
df_train_win = df_train_win.dropna().reset_index(drop=True)
df_valid_win = df_valid_win.dropna().reset_index(drop=True)
df_test_win = df_test_win.dropna().reset_index(drop=True)
...
model_pca = PCA(random_state=SEED)
model_pca.fit(train)

score_pca = model_pca.decision_function(valid)
result_pca = model_pca.predict(valid)
...
score_iforest = np.asarray(model_iforest.decision_function(valid.values)).ravel()
result_iforest = model_iforest.predict(valid.values)
# # -------------------------------------------------------
# kNNモデルの評価指標(検証)を計算
...
# PCAモデルの評価指標(検証)を計算
f1 = f1_score(valid_label, result_pca)
precision, recall, _ = precision_recall_curve(valid_label, score_pca)
pr_auc = auc(recall, precision)
roc_auc = roc_auc_score(valid_label, score_pca)
results.append({'Model': 'PCA', 'ROC_AUC': roc_auc, 'PR_AUC': pr_auc, 'F1': f1})

# iForestモデルの評価指標(検証)を計算
f1 = f1_score(valid_label, result_iforest)
precision, recall, _ = precision_recall_curve(valid_label, score_iforest)
pr_auc = auc(recall, precision)
roc_auc = roc_auc_score(valid_label, score_iforest)
results.append({'Model': 'IForest', 'ROC_AUC': roc_auc, 'PR_AUC': pr_auc, 'F1': f1})
...
# # # # -------------------------------------------------------
# F1スコアを最大化する閾値探索(knn)
precision, recall, thresholds = precision_recall_curve(valid_label, score_knn)
f1_scores = []

for thresh in thresholds:
    preds = (score_knn >= thresh).astype(int)
    f1_scores.append(f1_score(valid_label, preds))

best_thresh_knn = thresholds[f1_scores.index(max(f1_scores))]
best_f1_knn = max(f1_scores)
print("■kNN")
print(f"最大F1スコア: {best_f1_knn:.3f}, 最適閾値: {best_thresh_knn:.3f}")

# F1スコアを最大化する閾値探索(pca)
precision, recall, thresholds = precision_recall_curve(valid_label, score_pca)
f1_scores = []

for thresh in thresholds:
    preds = (score_pca >= thresh).astype(int)
    f1_scores.append(f1_score(valid_label, preds))

best_thresh_pca = thresholds[f1_scores.index(max(f1_scores))]
best_f1_pca = max(f1_scores)
print("■PCA")
print(f"最大F1スコア: {best_f1_pca:.3f}, 最適閾値: {best_thresh_pca:.3f}")

# F1スコアを最大化する閾値探索(iforest)
precision, recall, thresholds = precision_recall_curve(valid_label, score_iforest)
f1_scores = []

for thresh in thresholds:
    preds = (score_iforest >= thresh).astype(int)
    f1_scores.append(f1_score(valid_label, preds))

best_thresh_iforest = thresholds[f1_scores.index(max(f1_scores))]
best_f1_iforest = max(f1_scores)
print("■iFotrest")
print(f"最大F1スコア: {best_f1_iforest:.3f}, 最適閾値: {best_thresh_iforest:.3f}")

# kNNモデルの評価指標(テスト)を計算
# テストデータに対してスコアと予測を取得
score_knn = np.asarray(model_knn.decision_function(test.values)).ravel()
# score_knnとbest_threshを用いて異常予測を算出
result_knn = (score_knn >= best_thresh_knn).astype(int)

f1 = f1_score(test_label, result_knn)
precision, recall, _ = precision_recall_curve(test_label, score_knn)
pr_auc = auc(recall, precision)
roc_auc = roc_auc_score(test_label, score_knn)
results = [{'Model': 'kNN', 'ROC_AUC': roc_auc, 'PR_AUC': pr_auc, 'F1': f1}]

# PCAモデルの評価指標(テスト)を計算
# テストデータに対してスコアと予測を取得
score_pca = np.asarray(model_pca.decision_function(test.values)).ravel()
# score_pcaとbest_threshを用いて異常予測を算出
result_pca = (score_pca >= best_thresh_pca).astype(int)

f1 = f1_score(test_label, result_pca)
precision, recall, _ = precision_recall_curve(test_label, score_pca)
pr_auc = auc(recall, precision)
roc_auc = roc_auc_score(test_label, score_pca)
results.append({'Model': 'PCA', 'ROC_AUC': roc_auc, 'PR_AUC': pr_auc, 'F1': f1})

# iForestモデルの評価指標(テスト)を計算
# テストデータに対してスコアと予測を取得
score_iforest = np.asarray(model_iforest.decision_function(test.values)).ravel()
# score_iforestとbest_threshを用いて異常予測を算出
result_iforest = (score_iforest >= best_thresh_iforest).astype(int)

f1 = f1_score(test_label, result_iforest)
precision, recall, _ = precision_recall_curve(test_label, score_iforest)
pr_auc = auc(recall, precision)
roc_auc = roc_auc_score(test_label, score_iforest)
results.append({'Model': 'IForest', 'ROC_AUC': roc_auc, 'PR_AUC': pr_auc, 'F1': f1})
