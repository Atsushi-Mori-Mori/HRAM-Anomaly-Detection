...
# df_train1[cols] = df_train1[cols].apply(pd.to_numeric, errors='coerce')
# #1. 単位荷重あたりエネルギー
# df_train1['energy_per_ton'] = df_train1['energy_proxy_kNdeg'] / df_train1['peak_tonnage_kN'].replace(0, np.nan)
# #2. 電流効率指標
# df_train1['current_per_energy'] = df_train1['motor_current_A'] / df_train1['energy_proxy_kNdeg'].replace(0, np.nan)
# #3. 電流効率指標
# df_train1['vib_per_ton'] = df_train1['vibration_rms_g'] / df_train1['peak_tonnage_kN'].replace(0, np.nan)
# #4. BDC集中度
# df_train1['bdc_sharpness'] = df_train1['ftr_bdc_ratio'] / df_train1['ftr_halfwidth80_deg'].replace(0, np.nan)
# # 5. 速度補正電流
# df_train1['current_norm_spm'] = df_train1['motor_current_A'] / df_train1['spm'].replace(0, np.nan)
# # 6. 速度補正振動
# df_train1['vib_norm_spm'] = df_train1['vibration_rms_g'] / df_train1['spm'].replace(0, np.nan)
# # 7. 速度補正荷重
# df_train1['tonnage_norm_spm'] = df_train1['peak_tonnage_kN'] / df_train1['spm'].replace(0, np.nan)
# # 8. 移動平均との差
# # df_train1['delta_ton'] = df_train1['peak_tonnage_kN'] - df_train1['rolling_mean_50']
# # 9. 移動標準偏差比
# # df_train1['zscore_ton'] = (df_train1['peak_tonnage_kN'] - df_train1['rolling_mean']) / df_train1['rolling_std'].replace(0, np.nan)
# # 10. エネルギー×不規則度
# df_train1['energy_instability'] = df_train1['energy_proxy_kNdeg'] * df_train1['ftr_energy_irreg']
# # 11. 変動増幅指標
# df_train1['instability_index'] = df_train1['ftr_c2c_peak_delta_abs'] * df_train1['ftr_energy_irreg']
# # 12. 電流?荷重残差
# # motor_current_A ~ peak_tonnage_kN
# # 13. エネルギー?荷重残差
# # energy_proxy_kNdeg ~ peak_tonnage_kN
# # 14. 振動?荷重残差
# # 15. 勾配特徴量
# # slope_200 = 線形回帰傾き(直近200ショット)
# # tonnage_slope
# # energy_slope
# # vibration_slope
# # 16. 温度上昇率
# # temp_gradient = die_temp_C - rolling_mean_die_temp
# # 17. 潤滑余裕度
# # lube_margin = lube_pressure_bar - rolling_min_lube
# # 18. 機械リスク指数
# # machine_risk =
# #     vib_norm_spm
# #   + current_norm_spm
...
# #   * (vib_per_ton)
# # 21. 交差項
# # peak_tonnage_kN × vibration_rms_g
# # energy_proxy_kNdeg × motor_current_A
# # 22. 対数変換
# # log_peak_tonnage
# # log_vibration
# # 23. 比率対数
# # log(vibration_rms_g / peak_tonnage_kN)
# cols2 = [
#     "energy_per_ton",
#     "current_per_energy",
#     "vib_per_ton",
...
# # # 数値化（"－"などはNaN）
# # 1) '-' を NaN に置換（必要なら '－' や空文字も追加）
# df_train1[cols2] = df_train1[cols2].replace(["-", "－", ""], np.nan)
# # 2) 数値化（変換できないものはNaN）
# df_train1[cols2] = df_train1[cols2].apply(pd.to_numeric, errors="coerce")
# # is_anomaly も数値化（True/FalseでもOK）
# df_train1["is_anomaly"] = df_train1["is_anomaly"].replace(["-", "－", ""], np.nan)
# df_train1["is_anomaly"] = pd.to_numeric(df_train1["is_anomaly"], errors="coerce")
# # 3) 相関（各特徴量 vs is_anomaly）
# rows = []
# y = df_train1["is_anomaly"]
# for c in cols2:
#     x = df_train1[c]
#     m = x.notna() & y.notna()
#     n_valid = int(m.sum())
...
# )
# df_corr
# df_corr.to_csv("./data/train_corr2.csv", index=False) # index=Falseで行番号を除外
# # -------------------------------------------------------
# # -------------------------------------------------------
# # *******************************************************
# # -------------------------------------------------------
...
#     'z_lube_pressure_bar'
# ]
# def apply_ma_by_product(df, cols, window=5,
#                         group_col='product_type'):
#     df = df.copy()
#     df = df.sort_values(['product_type', 'timestamp'])
#     for col in cols:
#         df[col + '_ma'] = (
#             df
...
#     return df
# # 実行例（window=5）
# df_train_ma = apply_ma_by_product(df_train, colMA, window=5)
# df_train1 = df_train.copy()
# df_train = df_train_ma.copy()
# # -----------------------------
# # 異常値（'-'を含む行）の削除
...
import matplotlib.pyplot as plt
# 可視化する特徴量を指定（例）
features_to_plot = ['ftr_bdc_ratio', 'ftr_asymmetry', 'ftr_vib_high_band', 'peak_angle_deg',
                    'peak_tonnage_kN', 'vibration_rms_g', 'lube_pressure_bar',
                    'z_ftr_bdc_ratio', 'z_ftr_asymmetry', 'z_ftr_vib_high_band', 'z_peak_angle_deg',
                    'z_peak_tonnage_kN', 'z_vibration_rms_g', 'z_lube_pressure_bar',
                    'r_ftr_bdc_ratio', 'r_ftr_asymmetry', 'r_ftr_vib_high_band', 'r_peak_angle_deg',
                    'r_peak_tonnage_kN', 'r_vibration_rms_g', 'r_lube_pressure_bar']
...
#     if df_test_clean.iloc[i][anomaly_label] and not in_anomaly:
#         start_idx = i
#         in_anomaly = True
#     elif not df_test_clean.iloc[i][anomaly_label] and in_anomaly:
#         anomaly_periods.append((start_idx, i - 1))
#         in_anomaly = False
# # 最後が異常で終わった場合
# if in_anomaly:
#     anomaly_periods.append((start_idx, len(df_test_clean) - 1))
# # # -----------------------------
# import matplotlib.pyplot as plt
# # 可視化する特徴量を指定（例）
# features_to_plot = ['ftr_bdc_ratio', 'ftr_asymmetry', 'ftr_vib_high_band', 'peak_angle_deg',
...
# # ---- matplotlib で描画 ----
# # x軸（stroke_id）を numpy 配列にしておくと扱いやすい
# x = df_test_clean['stroke_id'].to_numpy()
# for feature in features_to_plot:
#     if feature not in df_test_clean.columns:
#         print(f"特徴量 '{feature}' がデータに存在しません。スキップします。")
#         continue
#     y = df_test_clean[feature].to_numpy()
#     fig, ax = plt.subplots(figsize=(12, 4))
#     # 異常区間を赤い背景で塗る（Plotlyのvrect相当）
#     for start, end in anomaly_periods:
#         x0 = df_test_clean.iloc[start]['stroke_id']
#         x1 = df_test_clean.iloc[end]['stroke_id']
#         ax.axvspan(x0, x1, alpha=0.2, color='red', linewidth=0)
#     # 時系列を描画（線＋必要なら点も）
#     ax.plot(x, y, linewidth=1, label=feature)
#     ax.set_title(f'訓練データ: {feature} の時系列推移')
